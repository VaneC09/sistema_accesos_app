...
class _QrResultWidgetState extends State<QrResultWidget> {
  bool _enviandoExtension = false;

  QrScanResultModel get resultado => widget.resultado;
  ...
  Future<void> _solicitarExtension() async {
    if (_enviandoExtension) return;

    setState(() => _enviandoExtension = true);
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(AppStrings.exitoSolicitudExtension),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(AppStrings.errorSolicitudExtension),
            backgroundColor: AppColors.actionRed,
          ),
        );
      }
    } finally {
      if (mounted) setState(() => _enviandoExtension = false);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final Color colorPrin =
        concedido ? AppColors.successGreen : AppColors.actionRed;
      ...
      child: Column(
        ...
        children: [
          ...
          if (!concedido) ...[
            ...
            if (resultado.puedeSolicitarExtension) ...[
              const SizedBox(height: AppSpacing.md),
              SizedBox(
                width: double.infinity,
                child: OutlinedButton.icon(
                  onPressed: _enviandoExtension ? null : _solicitarExtension,
                  icon: _enviandoExtension
                      ? const SizedBox(
                          width: 18,
                          height: 18,
                  ...
                  label: Text(
                    _enviandoExtension
                        ? 'Enviando...'
                        : AppStrings.botonSolicitarExtension,
                    style: const TextStyle(color: AppColors.primaryCoral),
                  ),
          ...
          if (widget.onNuevoEscaneo != null)
            TextButton.icon(
              onPressed: widget.onNuevoEscaneo,
              icon: const Icon(
